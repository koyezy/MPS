getwd()
setwd("/Users/yejiko/Downloads/MPS")
...
while(length(msm_est) < nsim){
  ...
  ### Generate A
  Aarray[1,] <- rlnorm(ns, mean = log(0.001 - 0.000015*1), 1) # intercept: 0.001 and slope: -0.000015, var= 0.01
  ...
  for(j in (2:tpoints)){
    ...
    ### Generate A
    Aarray[j,] <- rlnorm(ns, mean = log(0.001 - 0.000015*j), 1)
  ...
  mod <- glm(A ~ Am1 + time, data = final, family = "binomial")

  # calculate propensity score
  final$ps_treatment = (predict(mod, type = "response") *final$A) + 
                        ((1-predict(mod, type = "response")) * (1-final$A))
  
  mod_stab <- glm(A ~ Am1 + L + Lm1 + time, data = final, family = "binomial")
  summary(mod_stab)
  
  # calculate propensity score - stabilized weights
  final$ps_treatment_stabilized  = (predict(mod_stab, type = "response") *final$A) + 
                                    ((1-predict(mod_stab, type = "response")) * (1-final$A))
  ...
  mod <- glm(is.censored ~ Am1 + time, 
             data = final, family = "binomial")

  censoring_ps = (1-predict(mod, type = "response"))
  
  ### Denominator model includes baseline and time-varying covariates
  mod_stab <- glm(is.censored ~ Am1 + Lm1 + Lm2 + time, data = final, family = "binomial")
  
  stabilized_censoring_ps = (1-predict(mod_stab, type="response"))
  ...
  msm_se <- c(msm_se, summary(msm)$coef[1,3])
  ...
  #cph_ci <- rbind(cph_ci, confint(cph)[1,])
  ...
  #cph2_ci <- rbind(cph2_ci, confint(cph2)[1,])
}

write.csv(cbind(msm_est[msm_est<1 &msm_est>-1], msm_se[msm_est<1 &msm_est>-1], 
          cph_est[1:500], cph_se[1:500], cph_est2[1:500], cph_se2[1:500]), 
          "MPS_sim3_update.csv")

length(msm_est[msm_est<1 &msm_est>-1])

### Bias
round(mean(cph_est2[1:500]),4)-psi # MC mean for cox PH with no L
round(mean(cph_est[1:500]),4)-psi # MC mean for standard cox
round(mean(msm_est[msm_est<1 &msm_est>-1]),4)-psi # MC mean for cox MSM

### MSE
round(mean((cph_est2[1:500]-psi)^2),4)
round(mean((cph_est[1:500]-psi)^2),4)
round(mean((msm_est[msm_est<1 &msm_est>-1]-psi)^2),4)

### Coverage rate
lb =  cph_est2[1:500] - 1.96 *cph_se2[1:500]
ub =  cph_est2[1:500] + 1.96 *cph_se2[1:500]
cover <- ifelse(lb <psi & ub >psi,1,0)
mean(cover)

lb =  cph_est[1:500] - 1.96 *cph_se[1:500]
ub =  cph_est[1:500] + 1.96 *cph_se[1:500]
cover <- ifelse(lb <psi & ub >psi,1,0)
mean(cover)
